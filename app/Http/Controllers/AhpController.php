...
class AhpController extends Controller
{
    ...
    public function calculate(Request $request)
    {
        ...
        for ($i=1; $i <= count($items)-1; $i++) { 
            if ($i == $total_categories+2) {
                $k = $i;
                for ($j=$total_categories+3; $j < count($items); $j++) { 
                    $check = Ahp::where('decision_maker_id', $items[$k])
                            ->where('category_id', $items[$j])->get();
                    if(count($check) == null){
                        $ahp = new Ahp;
                        $ahp->decision_maker_id = $items[$k];
                        ...
                        $ahp->weight = $weight_normalized[$j-$total_categories-3];
                        $ahp->save();
                    }
                    else{

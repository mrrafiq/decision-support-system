...
@section('main')
    <div>
        <p class="text-4xl">Tambah Sekolah Pilihan</p>
    </div>
    ...
    <div class="mt-12">
        <div class="w-full bg-neutral-100 px-10 py-10 rounded-lg">
            <form action="{{ url('school-session/store/'. $session->id) }}" method="POST">
                {{ csrf_field() }}
                @for ($i = 0; $i < count($schools); $i++)
                    <div class="flex items-center mb-4">
                        <input type="checkbox"
                            ...
                            id="{{$schools[$i]->name}}" name="{{$schools[$i]->name}}" value="{{$schools[$i]->id}}">

...
@section('main')
    <div>
        ...
        @if ($name == null)
            <button
                ...
                <a href="{{ url('school/create-detail/' . $data->id) }}">Tambah</a>
            </button>
        @else
        <button
    ...
    <div class="mt-8 mb-12">
        ...
        <p class="text-xl font-semibold mt-8">{{ $categories == null ? '' : strtoupper(str_replace('_',' ',$categories[1])) }}</p>
        <p class="text-base">{{ $values == null ? '' : $values[1] }}</p>
        ...
        <p class="text-base">{{ $values == null ? '' : ucwords(str_replace('_',' - ',$values[3])) }}</p>
        <p class="text-xl font-semibold mt-8">{{ $categories == null ? '' : strtoupper(str_replace('_',' ',$categories[4])) }}</p>
        <p class="text-base">Rp.{{ $values == null ? '' : number_format((float)($values[4]), 2, ",", ".") }}</p>
